# -*- coding: utf-8 -*-
"""
Created on Tue Apr 19 21:42:47 2022
...
import os
os.chdir(r"C:\Users\howar\OneDrive\桌面\code\cache")
from post_processing import adjust_price_, auto_marking_, parse_price
...
def get_attributes(soup):
    ...
    try:
        result["連絡電話"] = soup.select_one("div#rightConFixed span.tel-txt").text.strip()
    except AttributeError as e:
        result["連絡電話"]=[]
    try:
        result["性別要求"] = ("限女生" if "限女生租住" in soup.select_one("div.service-rule").text else  "男女皆可")
    except AttributeError:
        result["性別要求"] = None
    result["城市"] = "Taipei"
    try:
        result["屋主自行刊登"] = (
            "yes" if "屋主" in soup.select_one("div#rightConFixed p.name").text else "no"
        )
    except AttributeError:
        result["屋主自行刊登"] = None
...
def main(
    source_path: str = "C:/Users/howar/OneDrive/桌面/code/cache/listings.jbl",
    data_path: Optional[str] = None,
    output_path: Optional[str] = r"C:\Users\howar\OneDrive\桌面\code\cache\591_New Taipei City.csv",
    limit: int = -1,
    headless: bool = False,
):
    ...
    if output_path is None and data_path is None:
        # default output path
        output_path = "C:/Users/howar/OneDrive/桌面/code/cache/591_New Taipei City.csv"
    ...
    df_new[column_ordering].to_csv(output_path, index=False,sep="," ,encoding='utf-8-sig')
    print("Finished!")
